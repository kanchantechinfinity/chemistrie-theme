...
## 2026-09-22 — Pillars chain drag: real cursor-tracking, eased settle (24151f9)
User: dragging the Trust Signals/Pillars carousel "is not smooth." Root cause: the drag handler (initPillarsChain in assets/chemistrie.js) only tracked `dragDelta` silently during pointermove — nothing on screen moved while dragging. On release it either did nothing (under 40px) or instantly swapped card classes (over 40px, animated by the cards' own .65s transition). No visual feedback WHILE dragging is what read as "not smooth," independent of how eased the eventual snap was.
Fix: pointermove now applies `translateX(dragDelta * 0.55)` to `#pillarsChainStage` every frame via requestAnimationFrame, with explicitly NO CSS transition during the drag (a transition there would lag a frame behind the per-frame JS writes, feeling laggy rather than direct — this is the opposite of the usual "add a transition for smoothness" instinct). On release, a temporary `.is-settling` class (sections/pillars.liquid, `transition: transform .4s cubic-bezier(.2,.8,.2,1)`) eases the stage's transform back to 0, removed via setTimeout(420) after the transition completes. The existing per-card .65s transition still handles the actual slide-to-slide swap when the drag passes the 40px threshold — this only adds the missing "follows your cursor while held" feedback layer on top.
**Pattern worth remembering**: for "not smooth" drag complaints, check whether the drag actually produces PER-FRAME visual feedback at all before assuming it's an easing/timing-curve problem — a transition genuinely hurts a live per-frame-updated transform, it only helps a one-shot settle/snap after input stops.

## 2026-09-23 — Reduced top padding: Collection teaser + Brand Story sections (05c4a98)
Two requests in one turn: "put [The Collection teaser] section slightly up... reduce the upper padding" (sections/shop.liquid, the "— The Collection — / Individual formulas. Your ritual." homepage block) and, mid-turn, "reduce this empty space above timeline" (sections/story.liquid, the brand-story chapters section, id `our-story`).
Both `.shop` and `.story` get their top padding from the shared `--section-pad-y` (clamp(80px,12vw,160px), used across many sections site-wide) — didn't touch the variable itself, scoped an override in each section's own `{% stylesheet %}` block instead.
**Used compound id+class selectors** (`#shop.shop`, `#our-story.story`), not bare `.shop`/`.story` — this project already hit and documented this exact pitfall once (sections/main-product.liquid's `.page-section.mprod` comment): a same-specificity class-only override in a section's own stylesheet is NOT reliably guaranteed to beat the shared base rule in chemistrie.css just because it's "the section's own file" — Shopify's section-CSS load order isn't a safe assumption to build on. Compound id+class raises specificity so it wins regardless of load order.
`#shop.shop` padding-top: clamp(40px,6vw,80px). `#our-story.story` padding-top: clamp(36px,5vw,72px).
**Pattern to reuse going forward**: any future "reduce padding on section X" request where X's padding comes from `--section-pad-y` (grep for it in chemistrie.css first) should get this same compound-selector scoped-override treatment, not a bare class rule.

## 2026-09-23 — 2011 story chapter photo (5a5ef7e)
User uploaded a photo (two men at a formal event, presumably Zach and Harin) to use for the homepage story timeline's "2011 / The Meeting" chapter (chapter_1 in sections/story.liquid). That chapter was falling back to `founder-1.jpg` — checked first and found that filename is SHARED with sections/founders.liquid (Pharmacists page founder bio) and sections/product-details.liquid, so overwriting it would have silently changed images on those other pages too. Saved the new photo as `assets/story-chapter1-2011.jpg` instead and pointed `chapter_1_image_asset` at it in templates/index.json, leaving founder-1.jpg untouched.
**Pattern to reuse**: before overwriting any existing theme asset filename to "replace an image," grep for that filename across the repo first — shared/reused asset files are common here (product images, founder photos), and overwriting one can silently change an unrelated section.

## 2026-09-23 — Fix: hero CTA stuck at low opacity, not a color bug (8a6112f)
User sent an actual screenshot this time (not just "it's missing"): the "Explore the Collection" button rendered as a barely-visible dark blend, not the solid tan pill it should be. Verified `--c-tan` (#c8aa7a) and `--c-forest` (#1a3a2d) are correct in :root — ruled out a CSS color bug. The visual (a muted dark-olive blend of tan+background, faint text) is the signature look of an element frozen partway through `gsap.from({opacity:0})` rather than ever reaching opacity:1 — consistent with the timing-fragility flagged in the previous investigation ([[31be33a]]-ish prior commit) but now with visual confirmation it's a real stuck-partial-opacity state, not just "slow to appear."
Fix: dropped `opacity` from both `.hero__cta-row` and `.hero__trust`'s `gsap.from()` entrance tweens in assets/chemistrie.js, keeping only the `y` slide-in. CSS default opacity is 1, so these are now unconditionally visible regardless of whether the GSAP tween ever runs or finishes — worst case on tween failure, they just appear in their final position with no slide, never invisible/faint.
**Pattern to generalize if this recurs elsewhere**: any `gsap.from({opacity:0, ...})` on a CTA/critical element is a latent "stuck invisible" risk if the tween is ever interrupted (tab backgrounded, GSAP CDN slow, ScrollTrigger conflicts, etc.) — prefer animating `y`/`scale` only for elements that must never fail to be visible, reserving opacity fades for decorative/non-critical elements.

## 2026-09-23 — Reduced padding between Founders and Active Index sections (76e9b18)
User sent a screenshot of an unlabeled section boundary (two flat cream colors, no text) and said "reduce the padding here" — no section name given. Identified it by sampling exact pixel colors via PowerShell (`[System.Drawing.Bitmap].GetPixel`): top = #E6D9C8, bottom = #F0E8DB, an exact match for `--c-cream-2` and `--c-cream`. Grepped chemistrie.css for full-section `--c-cream-2` backgrounds (3 candidates: `.pillars`, `.founders`, `.cta`), then checked templates/index.json's homepage `"order"` array to find which cream-2 section is directly followed by a cream section — `founders_cch8nz` → `actives_WHF8yt` (`.actives { background: var(--c-cream); }`) was the only adjacent match.
Fix: neither section had its own `{% stylesheet %}` block yet — added one to each. `.founders`'s bottom padding (shared `--section-pad-y`, clamp 80-160px) was stacking with `.actives__intro`'s top padding (clamp 48-88px). Reduced both: `#founders.founders` padding-bottom → clamp(40px,6vw,80px), `#actives .actives__intro` padding-top → clamp(24px,3vw,44px). Used compound id+class/id-descendant selectors per the established guaranteed-specificity pattern ([[chemistrie-theme-repo]] — same reasoning as `.page-section.mprod`).
**Technique worth reusing**: when a screenshot shows only flat color with no identifying text/content, sample exact pixel RGB via PowerShell `System.Drawing` and match against the theme's CSS custom-property hex values, then cross-reference the page's section order JSON for adjacency — reliable way to identify an unlabeled section boundary without guessing.

## 2026-09-23 — Padding reduction extended to all homepage sections (52682e2)
Follow-up to the Founders/Active-Index fix ([[76e9b18]]): user said "do it for all sections." Went through the full homepage order (hero → pillars → ritual → shop → founders → actives → proof → story → journal_preview → instagram → newsletter_cta) and applied the same scoped top/bottom padding reduction to every section still using the large shared padding, filling in whichever side (top or bottom) hadn't already been addressed:
- #shop.shop, #founders.founders, #our-story.story: added the missing complementary side (each already had one side done from earlier fixes)
- #ritual-finder.ritual-finder, .page-section.jpreview, #instagram.insta, .cta.cta--circle: new scoped overrides, none had one before
**Deliberately left alone**: `.pillars--chain` already has its own tuned override from an earlier session (top already tight, bottom is `clamp(80,9vw,120) !important` — a deliberate choice for the chain carousel's shadow/scale breathing room, not an oversight) and `.proof` already got a dedicated scoped pass earlier (`clamp(56,7vw,92)`, not the raw shared value). Didn't touch either since they're not the "someone forgot to address this" case the blanket request was really about — re-shrinking an already-deliberately-tuned `!important` value without new signal would likely undo a considered choice.
All new overrides use the established guaranteed-specificity pattern (compound id+class, or two classes already present on the element) rather than a bare class, per the `.page-section.mprod` precedent.

## 2026-09-23 — Fix: homepage hero image cropped (again), narrower single-property fix (017eb73)
User screenshot showed the hero product-lineup photo cropped on the right (Cashmere bottle cut tight against the edge). Measured empirically (PowerShell `System.Drawing`) rather than guessed: `assets/hero-home.jpg` is 1024x493, real aspect ratio 2.077. `.hero__visual-frame`'s box was `aspect-ratio:16/9.5` (1.684) with `object-fit:cover` — math confirms that crops ~9.5% off each side.
**Important context**: a hero-image "fix" was attempted earlier this project (wider grid columns + object-fit:contain + 4:3 ratio) and was explicitly reverted by the user (commits f854690/e07c4a0, reverted in efbb4d6) — so this needed a genuinely different, narrower approach, not a repeat of the reverted bundle. Changed ONLY `aspect-ratio` on `.hero__visual-frame` from `16/9.5` to `16/7.8` (≈2.051, closely matching the real photo) — kept `object-fit:cover` (no letterbox bars, avoids the "green extra bg" complaint from the reverted attempt) and did NOT touch `.hero__inner`'s grid-template-columns this time. Also fixed the `max-width:700px` mobile override, which had the same stale `16/9.5 !important` ratio and would have kept cropping on phones otherwise.
**Pattern to remember**: when a fix for X was explicitly reverted before, and X resurfaces as a complaint, don't repeat the same reverted bundle of changes — isolate the ONE property that actually fixes it and change nothing else the revert also undid.

## 2026-09-23 — Hero exact-ratio (no crop) + Collection two-prompt CTA redesign (57c569b, 28c04d6)
Two requests: (1) "KEEP THE original size of image dont crop it" — the previous 16/7.8 approximation still technically cropped ~1%. Changed `.hero__visual-frame`'s aspect-ratio to the fallback image's EXACT pixel ratio (1024/493, matching assets/hero-home.jpg precisely) on both the desktop rule and the max-width:700px `!important` mobile override — box and photo now identical shape, object-fit:cover has nothing to crop. If the merchant ever sets a differently-shaped image via the `hero_image` picker, this exact-match guarantee only holds for the current fallback file, not any future upload — flagged as a known limitation, not fixed further since no differently-shaped image exists yet to design around.

(2) Collection page "redesign this two ctas" — copy given ("Not sure where to start? Begin with The Ritual. →" / "Looking for a gift? See our curated sets. →") didn't exist ANYWHERE in the repo (grepped main-collection.liquid, ritual-finder-card.liquid, collection.json, and the concurrent `.kilo` agent's worktree — only match was the old single-CTA card's differently-worded eyebrow "NOT SURE WHERE TO START?"). Asked via AskUserQuestion; user said "just the cta nothing else" — took that as: stop clarifying, build it. Redesigned `snippets/ritual-finder-card.liquid` from one heading+body+button card into two stacked question/answer link rows (each full-row clickable, arrow slides on hover, hairline divider between). Updated `sections/main-collection.liquid` schema: `ritual_finder_eyebrow/heading/body/cta` → `prompt1_question/answer` + `prompt2_question/answer/url` (kept `ritual_finder_url` for prompt 1's destination, unchanged default `/pages/the-ritual`).
**No "curated sets"/gift collection exists yet** — `prompt2_url` defaults to `/collections/all` (never a dead link) with a schema `info` note telling the merchant to repoint it once a real gift-sets collection exists. Flagged, not fabricated.

## 2026-09-23 — Product page multi-image gallery, Foxtale-referenced, Aura first (21280e2)
Task: match https://foxtale.in/products/vit-c-gluta-advanced-brightening-serum's product gallery mechanics on all Chemistrie product pages. Inspected the reference screenshot closely before building — it's actually two things: (1) main image + prev/next arrows, thumbnail strip below + its own scroll arrows (click thumb swaps main image), and (2) a marketing text panel beside the main image with invented-sounding claims ("Non-Oxidizing Vitamin C Complex", "Stable. Potent. Effective.", "*Based on clinical studies"). Asked before building since this was meant to be the reusable template for every future product — user chose gallery mechanics only, no side text panel (no fabricated marketing copy).
- New snippets/product-gallery-images.liquid: mirrors product-image-asset.liquid's exact handle/title matching pattern, outputs a comma-separated theme-asset filename list per product, blank for products without a gallery yet (clean fallback to the existing single-image display — verified other 4 launch products untouched).
- Gallery order for Aura: existing product-aura.jpg (the same clean bottle shot already used on collection/homepage, for brand consistency) first, then the 4 uploaded lifestyle/texture photos (gallery-aura-1..4.png).
- sections/main-product.liquid: added a `gallery_list` check ahead of the existing `card_asset`/`product.images`/stock fallback chain — doesn't touch that chain's logic for any other product. New `.mprod__thumbstrip` (thumbnails + side scroll buttons, horizontal `overflow-x:auto`) is a separate CSS/JS path from the pre-existing plain-wrapping `.mprod__thumbs` (still used by the product.images fallback for non-gallery products) so they can't collide. The existing thumb-click-to-swap and main-image prev/next JS already queried `.mprod__thumb`/`.mprod__nav` generically — reused as-is, only added new JS for the thumbnail strip's own scroll-left/right buttons.
**Real Shopify product images were not an option** — the connected Shopify store (confirmed earlier this session, [[chemistrie-theme-repo]]) is an unrelated bedsheet store, not Chemistrie, so no real "Aura" product exists to upload images to. Used theme asset files instead, same mechanism as the existing single approved product photos.
**Next**: user will provide gallery images for the remaining 4 products (Velvet, Veil, Cashmere, Silken) — when they do, just add matching `echo` branches to product-gallery-images.liquid and save the files, no other changes needed; the main-product.liquid logic already generically supports any product this snippet returns a list for.

## 2026-09-23 — Fix: Shopify GitHub sync validation errors (8178f02)
Shopify's theme sync reported 4 failures (2 distinct issues) after the Collection CTA redesign and product gallery commits:

1. **`url`-type schema settings can't have a `default` in this theme's Shopify version.** Error: "setting with id=ritual_finder_url default must be a string or datasource access path." Grepped every `"type": "url"` setting across the whole theme — every single other one (privacy_url, instagram_url, pdf_url, product_url, etc.) omits `default` entirely. `ritual_finder_url` (pre-existing, not introduced today) and my new `prompt2_url` were the only two with a `default`, both now-invalid. Fixed by removing `default` from the schema and relying on the Liquid template's own `| default: '...'` filter at the point of use (already present in snippets/ritual-finder-card.liquid) — zero behavior change, just moved the fallback to where every other url setting in this theme already puts it.
...
## 2026-09-23 — Inner-page hero heading matched to homepage size (5c22fea)
User wanted the homepage hero's heading font size applied to "all other pages hero section." Confirmed via grep that one shared section, sections/page-hero.liquid, powers all 6 inner-page heroes (collection.json, page.contact.json, page.founders-circle.json, page.our-story.json, page.the-pharmacists.json, page.the-ritual.json) — a single change there covers every page at once. It was inheriting the shared `.display-h` clamp(40px,6vw,88px); scoped down to clamp(36px,4.6vw,66px) to match `.hero__title`'s current homepage size, using `.display-h.page-hero__title` (compound of two classes the h1 already carries) rather than a bare class, since it's a cross-file override against chemistrie.css.

## 2026-09-23 — Announcement bar turned on as an auto-scrolling marquee (ec868c8)
Task: add "PHARMACIST-FORMULATED SKINCARE, THOUGHTFULLY MADE." to the announcement bar, auto-moving marquee, site color palette. Found the bar already existed (sections/header.liquid `.utility-strip`, controlled by `config/settings_data.json`'s header section settings) — off by default since launch per an earlier brief, with default copy already exactly matching what was asked (`text-transform:uppercase` already renders it in caps, so no copy edit needed).
- Turned it on: `show_announcement: false -> true` in config/settings_data.json.
- Converted the static centered `<span>` into a marquee: 8 repeated copies in a flex track (`.utility-strip__track`), only the first copy is screen-reader-visible (rest `aria-hidden`, container carries the real `aria-label`), `translateX(0 -> -50%)` on an infinite 26s loop. With an even repeat count, -50% always lands exactly on an item boundary, so the loop has no visible seam/gap even on ultra-wide screens. Respects `prefers-reduced-motion` (animation removed, falls back to static text).
- Colors: kept the bar's existing `var(--c-forest)` background / `var(--c-cream)` text (already the site's established dark-bar palette here), added a small `var(--c-tan)` bullet between repeats.

## 2026-09-23 — Reduced shared --section-pad-y site-wide (5278297)
User: "theres lot of padding in between all the sections so reduce it" — generic, not homepage-specific. The earlier per-section fixes ([[76e9b18]], [[52682e2]]) only touched sections I'd individually identified; many others (`.vision`, and every `.page-section` user in pages.css — contact-main, contact-steps, main-collection, etc. — across the inner pages) were still on the original shared `--section-pad-y` (clamp(80px,12vw,160px)). Reduced the ROOT variable itself instead of hunting down each remaining section: clamp(80,12vw,160) -> clamp(40,6vw,80) (same ~50% ratio as the earlier per-section fixes), and its max-width:700px override clamp(56,10vw,80) -> clamp(32,6vw,48).
Confirmed sections with their own scoped fixed-value override (shop, founders, story, ritual-finder, jpreview, insta, newsletter-cta, proof — all from earlier work) are unaffected, since their compound-selector rules use fixed clamp values, not the variable, so nothing gets double-reduced.
**Pattern to remember**: when a padding-reduction request is stated generically ("all sections") rather than naming one, check whether the excess comes from a SHARED variable before doing another per-section sweep — reducing the root once is more reliable than chasing every remaining consumer individually, and avoids missing ones you didn't know existed (like `.vision`).

## 2026-09-23 — Site-wide type/image rescale + dead-space fix (b75ad83)
User: "lot of padding issue throughout the site as well as few sections look very huge and tacky so reduce the font size accordingly and also image size." Three screenshots: an enormous Founders heading, oversized pharmacist profile avatars, and a large blank gap between a dark section and a cream one.
**Root cause of "huge and tacky"**: `.display-h` — the shared section heading used across nearly every section — was `clamp(40px,6vw,88px)`, i.e. LARGER than the hero (66px). Mid-page section headings were out-competing the page's focal point. Cut to `clamp(26px,3.2vw,46px)` (line-height .98 -> 1.05 since it reads better small). Hierarchy is now hero 66 > section headings 46 > rest. Same pass on every other outlier (ritual-finder heading, proof numerals, story year, vision words, footer wordmark, both FAQ headings, contact-steps, story-opening lede, profile name, story watermark).
**Real bug found during the pass**: the `@media (max-width:1024px)` hero override was `clamp(64px,14vw,200px)` → ~133px at a 950px tablet width. My earlier audit regex missed it (didn't match 3-digit 200px). ALL four mobile/tablet hero overrides had drifted larger than the desktop base; they now step down monotonically 52/42/38/32. **Lesson: when auditing font sizes by regex, make sure the pattern covers 3-digit values — and check that mobile overrides are SMALLER than desktop, since overlapping breakpoints in this theme mean the last-in-source rule wins, not the narrowest.**
**Images**: `.pprofile__avatar` max-width 460 -> 330 (tablet 340->280, phone 240->220); `.founders__media` min-height 680 -> 460.
**Dead space (3rd screenshot)**: identified the boundary by sampling pixels (dark gradient above, `#F0E8DB`=`--c-cream` below) = Proof -> Story on the homepage. `sections/story.liquid` rendered `.story__intro` unconditionally, but eyebrow/heading/lede are all `""` on the homepage — an empty `.display-h` still reserves a full line box (~86px at the old size) plus margins. Now wrapped in blank checks, per-child and on the wrapper.
**Verified, not assumed**: `.pillar__title`/`.pillar__mono` are dead CSS (no liquid renders them) so left alone; every other section with possibly-blank fields (Instagram sub/body, Founders lede, page-hero deck2, page-cta sub2) already guards them.
**Tooling note**: chemistrie.css is CRLF, so multi-line string replacements in a Node script silently find 0 matches unless you use `\r\n`. Fall back to line-number-based edits (with a content assertion) for multi-line CSS blocks.

## 2026-09-23 — REVERTED the aggressive rescale; gentle reduction only (0622041, df6bc92)
**I over-reached and the user pushed back**: "bro u didnt have to change home page i had sent u ss all those sections u had to change revert ONLY THE HOME PAGE AND DONT REDUCE THE FONT SIZE ITNA ONLY REDUCE THE SIZE BY FEW NUMBERS."
What went wrong in b75ad83: asked to fix a few screenshotted sections, I instead halved the SHARED `.display-h` (88 -> 46) and reworked a pile of homepage-only sections (Story intro, Proof numerals, story years, Vision words, Founders media height, footer wordmark, all four hero breakpoints). Two compounding mistakes: (1) treating a request about specific screenshots as license for a site-wide redesign, (2) changing a global token by ~50% when "reduce" meant a nudge.
Fix: `git revert --no-commit b75ad83`, then re-applied only three small steps — `.display-h` 88->78, `.pprofile__avatar` 460->400, `.pprofile__name` 60->52. Verified with `git diff b75ad83~1` that the net change is exactly those three lines and everything else is byte-identical to the pre-commit state.
**Verified before reverting, worth keeping**: grepped which templates use each section — `story`, `founders`, `proof`, `vision` are homepage-only (index.json + page.home.json); `pharmacists-profiles` is the Pharmacists page. That's how I knew which changes were "homepage" and which could stay.
**Flagged, not fixed** (homepage, so left reverted until asked): the `@media (max-width:1024px)` rule is `.hero__title { font-size: clamp(64px,14vw,200px); }` → ~133px at a 950px tablet width.
**Then, same turn**: user sent a screenshot of ~200px of dead dark space under the nav. Found three rules far out of line with `.page-hero`'s 52px top padding: `.jhero` and `.art__header` (both clamp(120px,15vw,200px)) and `.page-hero--short` (clamp(110px,13vw,170px)) — the last one **redefined identically in 8 separate section files** (account, activate-account, addresses, cart, login, order, register, reset-password), so all 8 needed updating. Cut to ~72px / ~64px, bottom padding trimmed proportionally.
**LESSON (important)**: when the user sends screenshots of specific sections, change ONLY those sections. Do not generalise to shared tokens like `.display-h` or `--section-pad-y` unless they say "all"/"site-wide". And when they say "reduce", a nudge (~10%) is the default — confirm before making a >30% change to anything shared.

## 2026-09-23 — Product gallery hover zoom (1dc7b26)
User pointed at Foxtale's product page: hovering the main image shows a lens box following the cursor, with a magnified panel to the right showing exactly that region. (This also explains the translucent rectangle in the earlier Foxtale screenshot that I'd read as a static design element — it was the zoom lens.)
`WebFetch` on the live URL was useless here: it strips scripts and returns converted markdown, so it reported "no zoom functionality" even though the screenshot plainly shows it. **For JS-driven interactive behaviour, WebFetch can't confirm anything — work from the screenshot.**
Implementation in sections/main-product.liquid: `.mprod__lens` inside `.mprod__main` (clipped by its overflow), `.mprod__zoom` as a sibling inside the sticky `.mprod__gallery`, offset `left: calc(100% + grid gap)` so it lands over the buy-box column, square via `aspect-ratio:1/1` to match the image box. 2.6x magnification; panel `background-size`/`background-position` derived from the lens rect so lens and magnified view always agree.
**Key detail that would otherwise be a subtle bug**: the main image is `object-fit: contain` and our gallery photos are a mix of landscape/portrait, so the picture usually does NOT fill its element box. The code computes the actually-painted image rect from `naturalWidth/naturalHeight` (scale = min(boxW/natW, boxH/natH), centred) instead of using the element rect — without that the lens and zoom drift apart on every non-square image.
Gated off for `(hover: none)`, `(pointer: coarse)` and <=1024px (no room for the panel). Reads `main.currentSrc` at move time so it follows thumbnail swaps automatically.

## 2026-09-23 — Browser verification capability + fixed the zoom deadlock (d5bda75)
User: "i cant see the zoom on like images just as foxtale website" — the hover zoom I shipped in 1dc7b26 never worked.
**Real bug**: `track()` read `panel.clientWidth/clientHeight` to size the lens, but `.mprod__zoom` is `display:none` until the `is-zooming` class is added — and a `display:none` element reports 0. The guard `if (!pw || !ph) { hide(); return; }` therefore bailed on every pointer move, *before* the line that adds the class. Deadlock: couldn't be shown without a size, couldn't have a size without being shown. Fix: add the class (revealing the panel) BEFORE measuring it.
**Lesson**: never measure an element you're about to reveal — reveal first, then measure. And I shipped this without running it; the bug was trivially findable with one real page load.

### Browser access in this environment (set up, reusable)
No browser/Playwright MCP tool is connected to this session — `ToolSearch` for browser/navigate/screenshot returns only WebFetch (which strips JS and can't show interactive behaviour). But the machine has **Chrome at `C:\Program Files\Google\Chrome\Application\chrome.exe`** and **node v22 / npx**, so I installed **`puppeteer-core`** (not `puppeteer` — avoids the ~150MB Chromium download) into the session scratchpad at `…/scratchpad/browser/` and pointed it at the system Chrome via `executablePath`. Nothing installed into the repo.
**Harness pattern that worked well** for verifying a Shopify section without a live URL: extract the section's `{% stylesheet %}` and `{% javascript %}` blocks to files with node, extract `:root` vars from chemistrie.css, copy the real image assets, and write a `test.html` reproducing the section's markup. Then puppeteer: `page.mouse.move()` twice (one move alone may not fire the handlers as expected), read back `getComputedStyle`/`getBoundingClientRect` for the assertions, capture `pageerror`/`console` errors, and screenshot. Confirmed lens = panel/ZOOM and that the magnified region matches the lens.
**Reuse this** whenever a visual/interactive change needs verifying — far better than shipping and asking the user to check.

## 2026-09-24 — Product zoom: stacking, sharpness, 400x400 + accordion scroll (02dffcc)
1. **Zoom painted UNDER the buy-box text** (wishlist button, trust strip drawn on top of it). Cause: **`position: sticky` always creates a stacking context**, so `.mprod__zoom`'s `z-index:30` only ranked it *inside* `.mprod__gallery`; the buy box, a later sibling, painted over that entire context no matter how high the panel's own z-index went. Fix: give `.mprod__gallery` a z-index, raised to 20 only while `.is-zooming` (stays under the nav's 40). **Remember: raising a child's z-index can't escape an ancestor stacking context — raise the ancestor.**
2. **Zoom was blurry**: flat 2.6x magnification on 1536px-wide sources shown ~630px wide asks for ~1640px of source → browser upscales and interpolates. Fix: cap the factor at `naturalWidth / paintedWidth` so it never exceeds 1:1 native pixels (measured: background now exactly 1536px = naturalWidth, effective 2.47x, `upscaling: false`). Also raised the Shopify `image_url: width:` for the main image and thumbnail `data-full` from 1000 → 1800, so Shopify-hosted products have real detail to zoom into.
3. **Long accordion content unreachable**: `.mprod__acc-a.is-open` was `max-height:600px` + `overflow:hidden`, so "The Full Story" was cut off mid-sentence. Now `max-height:460px; overflow-y:auto` with contained overscroll and a slim scrollbar.
Panel sized to 400x400 per request.
**Verification gotcha worth remembering**: reading `getComputedStyle().maxHeight` right after toggling a class returns the *mid-transition* value (the panel animates max-height), which made the accordion look broken (`0px`) when it was fine — wait out the transition (~900ms) before asserting. Also: writing puppeteer scripts via bash heredoc mangles Windows path backslashes; use the Write tool for those files.
All three fixes verified in Chrome via puppeteer against a harness built from the section's real CSS/JS, plus `document.elementFromPoint` to prove the panel is genuinely topmost.

## 2026-09-24 — Product info accordion revamped (aacf3d5)
"revamp this whole section for all products" — the buy-box accordion (The Full Story / Warnings / The Feeling) in sections/main-product.liquid, shared by every product page.
**Direction chosen without asking, deliberately**: rather than invent a new look (which caused the earlier revert), matched the editorial FAQ pattern already approved and shipped on the Contact and Ritual pages (`.cfaq-item`/`.rfaq-item`) — hairline-divided rows instead of boxed cards, display-serif labels, the two-bar plus→minus morphing icon, sage accent on the open row. Consistency with an approved in-house pattern is a defensible direction; inventing one is not.
Also styled the panel's rich-text content (product descriptions carry their own headings/lists/FAQ and were falling back to browser defaults): serif sub-headings, ingredient bullets as tan dashes. Kept the scrollable overflow from 02dffcc. Class names unchanged (`.mprod__acc-*`) so the existing JS binding and `aria-controls` wiring keep working — only the icon span markup changed (3 identical occurrences swapped at once).
Verified in Chrome: `aria-expanded=true`, icon's vertical bar opacity 0 (plus→minus), label computes to the sage token, rows have 1px hairline borders, panel scrolls to the end, no console errors.

## 2026-09-24 — Product trust strip as badges + accordion heading recolour (bac69b1)
Two asks on all product pages (sections/main-product.liquid): revamp the "Pharmacist-formulated / Hand-numbered / Made in Houston" strip ("separate static button or marquee static or underline and highlight"), and change the accordion heading + hover colours.
- **Trust strip** → three static pill badges (rounded like the theme's buttons, cream fill, tan-3 border, small tan dot marker, full-strength `--c-forest` text instead of the 50%-opacity `--c-ink-mute`). Chose the "static button" option. Deliberately gave them NO hover/cursor/focus styling so they don't read as clickable.
- **Accordion headings**: `--c-forest` → `--c-forest-2` (deeper); hover accent `--c-sage` (washed out on cream) → `--c-tan-2` (bronze).
- **Deliberate asymmetry worth keeping**: hover recolours the whole label, but the OPEN row keeps the deep heading colour and tints only its icon. Bronze `#a78a5e` on cream `#f5edd9` is ~2.6:1 — acceptable for a transient hover, too low to leave a heading sitting in while the panel below it is being read. **When picking accent colours from this palette, check contrast: tan (#c8aa7a) and tan-2 (#a78a5e) both fail WCAG for sustained body/heading text on the cream backgrounds; they're fine as borders, markers, icons and transient hovers.**

## 2026-09-24 — Zach's real portrait replaces the placeholder (cd58bde)
"wherever you see Zach's name and next to it its picture, replace it." Traced every place Zach appears with a photo before touching anything:
- **Homepage Founders** (sections/founders.liquid): founder1 = Harin (`founder-1.jpg`), **founder2 = Zach (`founder-2.jpg`)**; no `founder2_image` set in index.json so the fallback asset renders.
- **Pharmacists page** (sections/pharmacists-profiles.liquid): the block has no `image` set, and the fallback **builds the filename dynamically** — `'founder-' | append: forloop.index0 | modulo: 3 | plus: 1 | append: '.jpg'` — which resolves to `founder-2.jpg` for Zach (second block).
**The dynamic construction is the trap**: a literal grep for "founder-2.jpg" returns only ONE hit (founders.liquid), because the Pharmacists page never spells the filename out. Had to read the markup. **Whenever checking "where is this asset used", grep for the filename AND for dynamically composed `| append:` filename patterns — this theme does both.**
So replacing the single asset covered both usages. Left `founder-1.jpg` alone (Harin, plus the generic founder photo in product-details.liquid whose alt is "One of Chemistrie's founding pharmacists", not captioned as Zach).
The old `founder-2.jpg` was a stock photo of a woman in a lab coat. Converted the supplied PNG → JPEG via PowerShell System.Drawing (quality 92, drawn onto a white bitmap first so PNG alpha can't render black) rather than renaming to .png, because the Pharmacists page composes the `.jpg` extension dynamically and a different extension would break it.

## 2026-09-24 — Announcement bar recoloured green-on-gold (30598ce)
Ask: keep the text green, take the background from the palette, revamp the design. Was cream-on-forest; now `--c-forest` text on `--c-tan` gold.
**Why gold and not a cream token**: the nav sits directly beneath the bar and is itself `--c-cream` — on cream/paper the bar would have blended into the nav entirely. Gold is the only palette token that both reads as a banner and separates from the nav. Measured the pairing instead of eyeballing: forest on tan = **5.62:1**, clears AA at this small size. (Useful reference alongside the earlier note that tan *text* on cream fails — tan works fine as a **background** for forest text, just not as foreground text on cream.)
Design refinements: bullet separators → 1px hairline rules (matching the FAQ/section divider language), letter-spacing .14→.2em, weight 400→500 (type needs more presence as dark-on-light than it did light-on-dark), 11→10.5px with a little more vertical padding, plus a hairline bottom border to seat it against the nav.
Verified in Chrome: tokens resolve correctly, separator 1px×11px, marquee still seamless (8 even repeats; half-track 1891px > 1400px viewport so no gap appears), no errors.

## 2026-09-24 — Harin's portrait + announcement bar re-recoloured (b8997b3, 91d5ddb)
**Harin photo**: same pattern as Zach ([[cd58bde]]) — `founder-1.jpg` was a stock woman-in-white-coat placeholder. Replaced (PNG→JPEG via System.Drawing, quality 92, flattened on white, keeping the `.jpg` name the Pharmacists page composes dynamically). One asset covers three surfaces: homepage Founders (founder 1), Pharmacists page (first block → `founder-1.jpg`), and the generic founder photo on every product page in product-details.liquid ("One of Chemistrie's founding pharmacists").
**Knock-on caught before shipping**: `templates/page.home.json` runs the Brand Story section with **no settings object at all**, so chapter 1's photo fell through to the schema default `founder-1.jpg`. Once that asset became Harin's face, the "2011 / The Meeting" chapter on that template would have shown his solo portrait instead of the two-of-them photo. Pointed it at `story-chapter1-2011.jpg` to match index.json. **Lesson: when replacing a shared asset, also check schema `default:` values pointing at it — a template with no settings silently inherits them.**
**Announcement bar**: user said the gold "looks trash". Moved `--c-tan` → `--c-sage-3` (pale sage). Keeps the bar in the green family with its forest text, still separates from the cream nav (cream/paper stay ruled out — the nav directly below is cream), and is much quieter. Contrast improved as a side effect: 8.59:1 vs 5.62:1 on gold.

## 2026-09-24 — Hero images capped at 600x600, uncropped (9b7efc1)
Both hero sections (`.hero__visual-frame` on the homepage, `.page-hero__visual` shared by the 6 inner pages) used a fixed `aspect-ratio` box + `object-fit: cover`, which crops anything not matching the box shape. Measured every hero asset first: ratios run 2.08 (hero-home, hero-pharmacists, hero-ritual) → 1.78/1.62 (our-story, contact) → 1.12 (stock-lineup) → 1.00 (hero-visual) → 0.75 portrait (stock-note). No single fixed box suits all.
Solution: absolute 600px cap on BOTH axes of the **image** with `object-fit: contain`, frame shrink-wraps it (`width: fit-content`, no aspect-ratio, `background: transparent`). Every photo shows in full, and because the frame hugs the image there are no letterbox bands inside the border.
**Important CSS trap hit here**: `max-width: min(600px, 100%)` on the image (or on a `fit-content` frame) **silently fails** — the percentage resolves against a parent whose own width depends on the image, and that circular dependency makes neither constraint apply. The widest photo rendered at its full 1024px despite both caps. Fix: absolute `max-width: 600px` on the image; handle small screens with a media query (`frame width:100%`, `img max-width:100%`) rather than a percentage in the base rule.
Verified in Chrome across widest/tallest/square/mid-landscape: all aspect-preserved, all within 600x600, frame within 2px of the image.
**Harness note**: slicing CSS out of chemistrie.css by hardcoded line numbers produced an orphan declaration fragment at the top of the extracted file, which broke parsing and gave a misleading first result — slice starting from a `findIndex` on a known comment instead.

## 2026-09-24 — Reverted the 600x600 hero sizing (9ea8d8b)
User: "undo it". Reverted 9b7efc1 — hero images are back to fixed aspect-ratio boxes with `object-fit: cover` (homepage `1024/493`, page-hero `16/9.5`, `--c-forest-2` frame background). Verified the diff against the pre-change commit is empty, so both files are byte-identical to their previous state. The 600x600/contain approach and the `min(600px,100%)` circularity finding are documented in the entry above if it's ever wanted again.
Same message asked why the announcement bar colour changed — it changed because it was requested twice: "change the announcment banner color theme keep the text color green and the background something from the color pallete" (→ green on gold, 30598ce), then "the background color of marque is looking so trash do something else" (→ green on pale sage, 91d5ddb). Left as-is; did NOT revert it, since that was a question rather than an instruction.

## 2026-09-24 — Collection hero lineup image + centred product thumbnails
**Collection hero** (`templates/collection.json`): new `assets/hero-collection.jpg` (1349x650 product lineup — Velvet, Silken, Aura, Veil, Cashmere) replaces the `stock-lineup.jpg` fallback.
The shared `.page-hero__visual` frame is `16 / 9.5` (1.684) + `object-fit: cover`, which would have sliced ~19% off each side of a 2.075-ratio banner — cutting Velvet and Cashmere out of frame. Rather than change the shared frame (6 pages use it), added an **optional per-section `visual_ratio` setting** in `sections/page-hero.liquid`, applied as an inline `aspect-ratio` on the frame. Blank = the standard 16/9.5 box, so nothing else moved. Collection is set to `1349 / 650`, matching the image exactly, so `cover` crops nothing.
Note `templates/*.json` are **LF**, while `sections/*.liquid` are **CRLF** — perl inserts must match the target file or you leave a mixed-ending line.

**Product thumbnail strip** (`sections/main-product.liquid`): user asked for it "slightly in middle". Added `justify-content: center` to `.mprod__thumbstrip` and to the base `.mprod__thumbs` (the wrapping fallback row), and `justify-content: safe center` on the scrolling `.mprod__thumbstrip .mprod__thumbs` track.
The `safe` keyword matters: a plain `center` on an `overflow-x: auto` flex container pushes the first item past the left edge where it can't be scrolled back to. `safe` falls back to start-alignment the moment the row overflows.
Verified in Chrome: 4-thumb row centres between the arrows; 15-thumb row start-aligns, first thumb visible at scrollLeft 0 and last reachable. Hero measured at frame ratio 2.069 vs natural 2.075 (rounding), nothing cropped.

## 2026-09-24 — Routine Placement redesigned (all products)
Was: two full-width cream slabs on the dark band, each a wrapping row of dark `nowrap` pills separated by tiny arrows, led by a mustard `#fbe6b8` "MORNING" tag that sat outside the palette.
Now: two cards (`.rplace__col`, max 860px, centred) holding a numbered step list (`.rstep`) — forest number badge, faint tan rail joining the badges, product name in `--ff-display` 18px, role as a 10px tan-2 caps label. AM/PM differ only by the icon disc colour (`--c-tan-3` / `--c-sage-3`), both icons forest ink, so the mustard is gone.
**Why the pills had to go**: `pd_routine_am/pm` copy is not uniform. Velvet/Veil/Aura use short roles — "(Cleanse)", "(Treat)" — but Cashmere and Silken carry sentence-length notes, e.g. "(Cashmere may also be layered in the evening if additional hydration is desired.)". Those were being forced into a `white-space: nowrap` pill.
New `snippets/routine-steps.liquid` splits each step on `(`, then branches on word count: <= 3 words renders as an uppercase role label, longer renders as a wrapping sentence caption. No per-product markup, and it degrades fine for Cashmere/Silken AM where the copy has no parentheticals at all.
...
## 2026-09-24 — Contact hero photo + Routine Placement rebuilt as an editorial card grid
**Contact hero**: `assets/hero-contact.jpg` replaced with the new desk/notebook photo (1349x650, same ratio as the collection hero). Added `"visual_ratio": "1349 / 650"` to `templates/page.contact.json`'s page-hero settings, same treatment as the collection hero fix — the shared 16/9.5 frame would have cropped ~19% off each side of this wider image.

**Routine Placement**: user shared a reference design (dark "Routine Placement" page — Syne/Space Mono/Cormorant, forest/gold/alabaster Tailwind palette, numbered step CARDS with ghost-numeral watermarks, staggered elevation, AM/PM meta header strips) and asked to rebuild with it but "keep the color palette and text content and font as it is... remove the unnecessary content."
Read that as: adopt the *structure* (individual elevated cards instead of a connected rail; a header strip per period with a badge + step count; a faint numeral watermark) but keep our own tokens (`--c-forest`/`--c-cream`/`--c-tan-2`/etc., `--ff-display` Cormorant + `--ff-body` Jost) and our real copy only — no fabricated dosage/timing text, no "Technical Bulletin" advisory footer, no "Morning/Evening Focus" summary strip, no "Show Detailed Notes" toggle, no AM/PM tab-switcher (that would have hidden content that's currently always visible, which reads as removing real content rather than removing unnecessary content).
`snippets/routine-steps.liquid` now renders `.rplace__cards` (a `repeat(auto-fit, minmax(150px,1fr))` grid) instead of `.rplace__track` (the rail). Each `.rplace__card` gets a `.rplace__ghost` numeral at `rgba(26,58,45,.06)` — same technique as the reference's `text-forest-900/[0.06]` on an alabaster card, which maps almost exactly onto our own forest-on-cream tokens. Long parenthetical notes (Cashmere/Silken) still drop to numbered footnotes below the grid, unchanged from the previous version.
`.rplace__head` gained a right-aligned `.rplace__meta` pill showing the step count (e.g. "4-Step Routine") — derived from `am_steps.size`/`pm_steps.size`, real data, not the reference's fabricated "~4 Minutes" estimate.
Added a light per-card stagger (`nth-child` `margin-top` 0/10/20/30px, desktop only) as a restrained nod to the reference's cascading offsets, much smaller than the reference's own (24-64px) so a 4-card row doesn't leave a tall gap under the shortest card.
**Found real file corruption while in this file**: `.rplace__step:not(:last-child)::after` comment from the earlier rail version contained a literal U+FFFD replacement character where an em-dash should have been (visible via `chr(0xFFFD) in s`), left over from some earlier encoding mishap. It's gone now since that whole block was replaced; checked no other U+FFFD remains in the file.
**Also discovered `sections/product-details.liquid` is now pure LF**, not CRLF as originally created — it must have been re-saved by the editor at some point this session (a system reminder flagged an on-disk change earlier in this task). `snippets/routine-steps.liquid` and `templates/page.contact.json` are LF too. `sections/main-product.liquid`, `sections/page-hero.liquid`, `sections/ritual-steps.liquid` are still CRLF — per-file detection remains necessary, don't assume from one file.
Verified in Chrome: card counts/colors/step-count meta all correct across all 4 routine shapes (4-step AM, 3-step PM, evening-only single period, long-note footnotes); no overflow; responsive down to 420px (grid collapses to one column). Confirmed the ghost numeral genuinely renders (isolated element screenshot) — it only looked absent for "1" in a heavily downscaled full-page screenshot because that glyph has thin ink coverage at 6% opacity, not a bug.

## 2026-09-24 — Quantity box was still full-width on single-variant products
User: "this is not fixed" — re-shipped the previous qty/CTA shrink with a screenshot showing quantity still spanning the full row.
Root cause found: `.mprod__row--options input[type="hidden"] ~ .mprod__qty { flex: 1 1 auto; }` (a pre-existing rule, not something the previous commit touched) matches whenever the product has no variant picker — those render a `<input type="hidden" name="id">` in place of `.mprod__variants`, right before `.mprod__qty`. That sibling-combinator rule stretches the box to fill the whole row, which silently overrode the new `flex: 0 0 108px` base rule for every single-variant product. Multi-variant products (with a real `.mprod__variants` dropdown next to the qty box) were never affected — they were already shrinking correctly, which is likely why this slipped through the first verification pass (the earlier browser test only checked the with-variants markup shape).
Fixed by giving the hidden-input case its own fixed width (`flex: 0 0 140px`, slightly wider since it's alone in the row) instead of `1 1 auto`.
Verified in Chrome with the actual single-variant markup shape (hidden id input + qty, no variant dropdown): box now renders at a fixed 140px instead of stretching, button padding/font confirmed still 13px/22px / 11px from the earlier fix (that part was already correct — no competing selector found anywhere in the theme with equal or higher specificity than `.btn.mprod__add`).

## 2026-09-24 — Add to Ritual button shrunk further
User: "shrink the ritual button too" — the first pass (13px/22px padding, 11px font, 42px tall) wasn't enough. `.btn.mprod__add` now padding 10px/18px, font-size 10px, letter-spacing tightened to .13em (from the inherited .15em, so the smaller text doesn't look overly spaced out). Height 42px → 35px. Verified in Chrome against the single-variant markup shape (same one the previous qty-box bug hid in).

## 2026-09-24 — Routine Placement rebuilt as an accordion (Concept 3)
User picked "3rd" from a list of revamp ideas (accordion / progress-bar / etc., shown as rendered mockups first before building). Replaced the card-grid version with `.racc` — Morning and Evening as independently-toggleable collapsible rows, reusing the exact same `aria-expanded` + `.is-open` + `max-height` transition pattern as the existing FAQ accordion (`.pfaq__`) further down this same file, rather than inventing a second toggle mechanism for the same job.
Default open state: Morning always starts open; Evening starts open only when it's the only period present (Aura, evening-only) — both open when there's just one, only Morning open when there are two, so the section rests shorter on multi-period products.
Panel content (`snippets/routine-steps.liquid`) simplified from the card grid to plain numbered rows (badge + serif product name + role tag), separated by hairlines, no per-step card chrome — matches the lighter, more minimal read of the chosen concept vs. the card grid it replaced. Long parenthetical notes (Cashmere/Silken) still drop to footnotes at the bottom of each panel.
Renamed the third naming generation `rplace__` → `racc__` (first was `rstep__`, collided with `sections/ritual-steps.liquid`; second was `rplace__`, fine but the card-grid concept it named is gone). Cleaned up a stray orphaned comment left over from the very first rail version that never got removed in the last rewrite.
**Test harness gotcha worth remembering**: first render of the interactive click-to-open test showed the class toggling on `.racc__item` but the panel itself never gaining `.is-open` — looked like a real JS bug. Root cause was the *test mockup* omitting `aria-controls`/matching `id` on the panel (the production Liquid markup has both), so `document.getElementById(btn.getAttribute('aria-controls'))` returned null and only the `closest('.racc__item')` half of the toggle ran. Fixed the test harness, re-verified: panel `max-height` correctly animates 0 → 640px and rect height reflects real content height after the click.

## 2026-09-24 — Routine Placement rebuilt again: Morning/Evening tabs + bento-stairs boxes
User asked for a heading + 2 tabs (Morning/Evening) + step "pointers in bento boxes in stairs feel" — shown as a rendered mockup first (tabs + boxes ascending left-to-right, alternating tint), approved, then built for real.
Replaces the accordion from earlier today. Tabs only render when a product has both periods (`routine_both`); single-period products (Aura) just show that one panel, no toggle — a toggle with one option is friction, not a feature.
Tab behaviour reuses the exact show/hide-by-`[hidden]` pattern already used by `sections/ritual-steps.liquid`'s own Morning/Evening switcher (that section has its own sitewide, non-per-product version of this same idea on the Ritual page) — same interaction, new class names (`rtabs__`/`rbento__`) so it can't collide if both sections ever land on one page, same reasoning as every earlier naming pass on this block.
Fourth class-naming generation for this feature: `rstep__` (collided with ritual-steps.liquid) → `rplace__` (card grid) → `racc__` (accordion) → `rbento__` (this one). Documented the lineage in the snippet's own comment so the next pass doesn't have to reconstruct it from git log.
Bento-stairs boxes: `flex:1 1 0` row, each box a bit taller than the last via `nth-child` padding-top (20/40/60/80px), alternating background tint (cream/tan-3/cream/sage-3) for the mosaic feel, large faint numeral watermark reused from the card-grid concept.
**Caught before shipping**: the desktop `flex:1 1 0` stairs layout breaks on mobile once `.rbento` switches to `flex-direction:column` — `flex-grow` on a now-vertical main axis makes every box fight to equally divide the column's height instead of sizing to its own content, squeezing the name/role text out of view entirely (not just visually cramped — actually invisible). Fixed with `flex: 0 0 auto` + `!important` on the nth-child padding at the same breakpoint (nth-child's specificity otherwise beats a plain class rule regardless of source order, so a plain override wouldn't have won). Documented in the CSS comment since this exact flex/media-query trap is easy to reintroduce.
Verified interactively in Chrome (real extracted CSS + JS, not a rebuilt mock): clicking the Evening tab actually hides the Morning panel and shows Evening's (`hidden` attribute flips, `aria-pressed` updates, active pill restyles) — not just a class toggle with no visible effect. All three routine shapes (both periods, evening-only, long-note footnotes) correct at desktop and mobile.

## 2026-09-24 — Bento boxes: same size at rest, stairs-feel moved to hover
User: "make it same size and on hover animation of up and down size alternate card". The static ascending stairs (padding-top 20/40/60/80px per box) is gone — `align-items: stretch` on `.rbento` now gives every box in a row the same height regardless of content length. The stairs motion moved to interaction instead: `.rbento__box:nth-child(odd):hover { translateY(-10px) }`, `:nth-child(even):hover { translateY(10px) }`, so hovering the row alternately lifts and dips each box rather than holding a fixed diagonal.
Background tint alternation (cream/tan-3/cream/sage-3) kept — that part wasn't asked to change, only the size/stairs.
Disabled the hover transform under 700px (`.rbento__box:hover { transform: none; }`) — boxes stack full-width there, and translating a full-width box up/down just reads as jitter, not a stairs motion; the alternating tint still gives the mobile stack visual rhythm without it. Respects `prefers-reduced-motion`.
Verified in Chrome: all boxes in a row report identical `getBoundingClientRect().height` (134px for 4-step rows, 106px for 3-step); hovering box 1 (odd) moves it up 10px and it relaxes back to rest when hover moves elsewhere; box 2 (even) moves down 10px on its own hover.

## 2026-09-24 — Bento box hover: color fill sweeping from the top-right corner
User: "on hover can I feel the color from top right corner of the card coming all over the card". Added `.rbento__box::before` — a circle 200% of the box's own size, anchored so its center sits at the top-right corner, `transform: scale(0)` at rest and `scale(1)` on hover (0.5s), sitting behind the content (`z-index: 0` vs content's `1`) and clipped by the box's existing `overflow: hidden` + `border-radius` so it never spills past the rounded corners mid-sweep.
**Caught before shipping, not by eye**: first pass filled with `var(--c-forest)`. This band's own background is `#1A3A2D`, and `--c-forest` is `#1a3a2d` — same color. Computed-style diffs looked correct (scale 0→1, right transform matrix), but the hovered card would have nearly vanished into the surrounding band instead of visibly filling with color, since fill and backdrop were almost identical. Only caught this by cross-checking the actual hex values, not by eyeballing a screenshot at a glance. Switched the fill to `var(--c-tan)`, the theme's established accent-on-dark-forest color (already used for exactly this job elsewhere: jhero__cat, jhero__read, the announcement bar).
Text colors reconsidered for the new tan fill (not a straight forest→cream invert like a dark fill would need): product name stays forest at all times (forest-on-tan already has good contrast, no flip needed); role label flips tan-2→forest on hover (tan-2 is a darker tan, too close in hue/lightness to the tan fill to stay legible); numeral badge still inverts forest/cream ↔ cream/forest, which works fine on tan too.
Verified in Chrome: computed `::before` background is `rgb(200,170,122)` (`--c-tan`) at both rest and hover, transform matrix 0→1 confirmed, role/num colors flip correctly, name stays legible throughout, mid-transition screenshot shows the sweep visibly in progress (not an instant snap).
